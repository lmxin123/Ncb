...
namespace Ncb.DataManager
{
    ...
    {
        public async Task<GeneralResponseModel<List<View_UserDeviceInfos>>> QueryAsync(UserInfoQueryViewModel model, int pageIndex, int pageSize, RecordStates state = RecordStates.AuditPass)
        {
            ...
            sqlBuilder.AppendFormat("FROM dbo.View_UserDeviceInfos WHERE RecordState={0} ", state);
            if (model != null)
            {
